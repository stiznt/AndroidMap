...
import ru.stiznt.mapinkotlin.Adapter.SearchAdapter
            ...
            override fun onTextChanged(s: CharSequence, start: Int, before: Int, count: Int) {
                ...
                if (command != "") {
                    ...
                    for (i in pos.indices) {
                        if (pos[i].name.toLowerCase().contains(command.toLowerCase())) filter.add(
                            pos[i]
                        )
                    }
                    searchAdapter = SearchAdapter(context, R.layout.list_comitem, filter)
